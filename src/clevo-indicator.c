...
#define EC_REG_CPU_TEMP 0x07 //works
#define EC_REG_GPU_TEMP 0xCD //switch off, so 0
#define EC_REG_FAN_DUTY 0xCE  // works
...
static gboolean ui_update(gpointer user_data) {
    char label[256];
//    sprintf(label, "%d℃ %d℃ %d", share_info->cpu_temp, share_info->gpu_temp, share_info->fan_duty);
    sprintf(label, "%d℃  %d%%", share_info->cpu_temp, share_info->fan_duty);
...
static int ec_auto_duty_adjust_orginal(void) {
    int temp = MAX(share_info->cpu_temp, share_info->gpu_temp);
    int duty = share_info->fan_duty;
    //
    if (temp >= 80 && duty < 100)
